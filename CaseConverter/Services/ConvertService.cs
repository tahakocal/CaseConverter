...
public class ConvertService : IConvertService
{
    ...
    private static Dictionary<string, string> _characterTable;
    ...
    public Task<string> ConvertSlugify(string name, bool convertNonWesternChars, bool allowUnicodeCharsInUrls)
    {
        ...
        if (convertNonWesternChars && _characterTable == null)
            InitializeSeoCharacterTable();

        var stringBuilder = new StringBuilder();
        foreach (var c in name)
        {
            var char2 = c.ToString();
            if (convertNonWesternChars)
                if (_characterTable?.ContainsKey(char2) ?? false)
                    char2 = _characterTable[char2];

            if (allowUnicodeCharsInUrls)
            {
                if (char.IsLetterOrDigit(c) || okChars.Contains(char2))
                    stringBuilder.Append(char2);
            }
            else if (okChars.Contains(char2))
            {
                stringBuilder.Append(char2);
            }
        }

        var name2 = stringBuilder.ToString();
        name2 = name2.Replace(" ", "-");
    ...
    {
        return Task.FromResult(name.ToUpper(culture));
    }

    public Task<string> ConvertLowerCase(string name, CultureInfo culture)
    {
        return Task.FromResult(name.ToLower(culture));
    }

    public Task<string> ConvertCapitalizedCase(string name, CultureInfo culture)
    {
        TextInfo textInfo = culture.TextInfo;
        string capitalizedCaseName = textInfo.ToTitleCase(name);

        return Task.FromResult(capitalizedCaseName);
    }
    ...
    protected virtual void InitializeSeoCharacterTable()
    {
        lock (_lock)
        {
            _characterTable = new Dictionary<string, string>
            {
                { ToUnichar("0041"), "A" },
                { ToUnichar("0042"), "B" },
                { ToUnichar("0043"), "C" },
